...
func TestIndexMode_CountsTheFilteredList(t *testing.T) {
	...
	require.Equal(t, []string{"~/code/app", "rails-app"}, filteredNames(m),
		"anything after the sigil filters and ranks as it normally would")

	result, _ := m.Update(tea.KeyPressMsg{Code: '2', Text: "2"})
	assert.Equal(t, "rails-app", result.(Model).Chosen(),
